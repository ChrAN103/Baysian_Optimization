...
class Tetris:
    ...
    @classmethod
    def _init_rotated_shapes(cls):
        """Pre-compute all possible rotations for each shape to avoid repeated calculations."""
        if cls.ROTATED_SHAPES:
            return

        for idx, shape in enumerate(cls.SHAPES):
            cls.ROTATED_SHAPES[idx] = []
            rotated = shape.copy()
            for _ in range(4):
                cls.ROTATED_SHAPES[idx].append(rotated)
    ...
    def __init__(self, columns: int, rows: int, use_level: bool = True):
        ...
        Tetris._init_rotated_shapes()

        self.board = np.zeros((self.rows, self.cols), dtype=np.uint8)
        self._temp_board = np.zeros((self.rows, self.cols), dtype=np.uint8)

        self.current_shape_idx = 0
        self.held_shape_idx = 0
    ...
    def __str__(self):
        ...
        lines.extend([str(row) for row in self.shape])
        lines.append("board:")
        lines.extend([str(row) for row in self.board])
    ...
    def _get_current_state(
        self, board: np.ndarray, shape: np.ndarray, offset: tuple
    ) -> np.ndarray[int]:
        np.copyto(self._temp_board, board)
        self._place_shape(self._temp_board, shape, offset)
        return self.heuristics.get_heuristics(self._temp_board)
    ...
    def _get_state(self, x, rotation, holding):
        ...
        y = self._soft_drop(self.board, rotated, (x, 0))
        return self._get_current_state(self.board, rotated, (x, y))
        ...
        for holding in range(2):
            for rotation in range(4):
                rotated = self._rotate(
                    self.board, self.shape, (self.shape_x, 0), rotation
                )
    ...
    def _rotate(
        self, board: np.ndarray, shape: np.ndarray, offset: tuple, times: int = 1
    ) -> np.ndarray:
    ...
    def _move(
        self, board: np.ndarray, shape: np.ndarray, offset: tuple, delta_x: int
    ) -> int:
    ...
    def _get_new_shapes(self):
        self.next_shapes.extend(
            [self._get_new_random_shape() for _ in range(4 - len(self.next_shapes))]
        )
    ...
    def _evaluate_position(self):
        if not self._check_collision(
            self.board, self.shape, (self.shape_x, self.shape_y)
        ):
            return
    ...
    def rotate(self, times: int = 1):
        if self.done:
            return
        self.shape = self._rotate(
            self.board, self.shape, (self.shape_x, self.shape_y), times
        )

    def move(self, delta_x: int):
        if self.done:
            return
        self.shape_x = self._move(
            self.board, self.shape, (self.shape_x, self.shape_y), delta_x
        )
