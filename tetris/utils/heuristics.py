...
class Heuristics:
    ...
    def get_heuristics(self, board: np.ndarray):
        ...
        min_height, mean_heights, max_height, bumpiness = (
            self._calculate_max_height_and_bumpiness(board)
        )
    ...
    def _get_heights(self, board):
        return np.max(
            np.where(board != 0, len(board) - np.arange(len(board))[:, None], 0), axis=0
        )
